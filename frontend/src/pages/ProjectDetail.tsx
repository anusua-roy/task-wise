...
export default function ProjectDetail({ onSignOut }: { onSignOut?: ()=>void }) {
  const { id } = useParams();
        ...
        <div className="mb-6">
          <div className="flex items-start justify-between">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-2 rounded-lg border border-border" onClick={()=>navigate(-1)}>Back</button>
              <button className="px-3 py-2 rounded-lg bg-orange-600 text-white" onClick={()=>alert('Add Task')}>
                Add Task
              </button>
              {onSignOut && (
                <button className="px-3 py-2 rounded-lg border border-border" onClick={() => { onSignOut(); navigate('/signin'); }}>
                  Sign out
                </button>
              )}
            </div>
